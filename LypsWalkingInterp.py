from LypsImpl import LSymbol, LList, LMap, LFunction, LPrimitive
...
class LypsWalkingInterpreter( object ):
   def __init__( self ):
      self._codeSeg = None
      self._env     = Environment( parent=None, primitives=self._constructPrimitives() )
      self.reboot( )
   ...
   def _constructPrimitives( self ):
      ...
      class LDefPrimitive( object ):
         ...
         def __call__( self, primitiveDef ):
            ...
            primitiveList[ self._name ] = lPrimitivObj
            return lPrimitivObj
      ...
      # ###################################
      LNULL = LList( )
      primitiveList[ 'NULL' ] = LNULL
      primitiveList[ 'PI'   ] = math.pi
      primitiveList[ 'E'    ] = math.e
      primitiveList[ 'INF'  ] = math.inf
      primitiveList[ '-INF' ] = -math.inf
      primitiveList[ 'NAN'  ] = math.nan
      ...
      def LP_defGlobal( env, *args, **keys ):
         ...
         try:
            if isinstance( args[1], LFunction ):
               args[1].rebind( args[0]._val )
   
            return env.bind_g( *args )
         except:
